...
from mocap_spline_calculator import Spline_Calculator
...
# from mocap_msg.msg import RigidInfo
# mocap_msg/msg/RigidInfo.msg
#   int32 id
#   geometry_msgs/Pose rigid_pose
#   bool tracking_valid_flag


def trace(*args):
    pass  # print( "".join( [str(a) for a in args] ) )
...
class NatNetClient:
    def __init__(self):
        ...
        #rigid_body_list = ["tb0", "tb1","tb2", "ob0"]
        
        # rigid_body_list = ["tub0", "tub1","tub2","tub3","re0","re1","re2","re3","re4","re5"]
        
        rigid_body_list = ["test0"]
        
        #rigid_body_list = ["tub0","re3"]
        print("ROS node initializing ...")
        self.pub_list = [
            rospy.Publisher(
                "mocap_pose_topic/{0}_pose".format(rigid_body_list[i]),
                PoseStamped,
                queue_size=1,
            )
            for i in range(len(rigid_body_list))
        ]
        #for vel publisher node
        self.pub_twist_list = [
            rospy.Publisher(
                "mocap_pose_topic/{0}_twist".format(rigid_body_list[i]),
                Twist,
                queue_size=1,
            )
            for i in range(len(rigid_body_list))
        ...
        # Create a index for making state buffer (for rigidBodyVelcity())
        self.state_buffer_index = np.zeros(len(rigid_body_list), dtype=int)
        #the paramiter for the spline method, then you should select the ideal number
        self.state_buffer_index_len = 10
        self.state_buffer=np.empty((len(rigid_body_list),self.state_buffer_index_len),dtype=object)
        #ipdb.set_trace()
    ...
    def transform_for_xaxis(self,x,y,z,w):
        euler_list=quaternion_to_euler([x,y,z,w])
        euler_list[0]-=(math.pi/2)
        quat=euler_to_quaternion(euler_list)
        return quat 
    ...
    def rigidBodyListener(self, id, position, rotation, trackingValidFlag):
        ...
        br.sendTransform(
            ...
            (self.transform_for_xaxis(rotation[0], rotation[1], rotation[2], rotation[3])),
            rospy.Time.now(),
            self.pub_list[id-1].name[17:]+"/base_footprint",
            # "mocap_world")
            # "world")
            "map")
        ...
        # rigid_info.id = id
        # rigid_info.rigid_pose = rigid_pose
        # rigid_info.tracking_valid_flag = trackingValidFlag

        # # if id==1:
        # self.pub.publish(rigid_info)
        ...
        rigid_posestamped.header.frame_id = "map"



        # velocity_lsit=self.rigid_body_buffer_making(rigid_posestamped,id)
        # velocity_list is [linear_x,linear_y,angular_z]
        
        #(velocity)++++++++=self.rigid_body_buffer_making(rigid_posestamped)
        #ipdb.set_trace()

        twist = Twist()
        #twist.linear.x = velocity_lsit[0]
        #twist.linear.y = velocity_lsit[1]
        #twist.angular.z = velocity_lsit[2]*(-1)

        self.pub_list[id - 1].publish(rigid_posestamped)
        self.pub_twist_list [id - 1].publish(twist)


    def rigid_body_buffer_making(self,rigid_posestamped,id):
        # the variable is for the velocity calculation of the rigid body
        if self.state_buffer_index[id-1]<self.state_buffer_index_len:
            self.rigidBodyVelocity(rigid_posestamped,self.state_buffer_index[id-1],id)
            self.state_buffer_index[id-1]+=1
            return [0,0,0]
        else:
            rigidbody_velocity = self.rigidBodyVelocity(rigid_posestamped,self.state_buffer_index[id-1],id)
            #print(f'state_buffer_index is {self.state_buffer_index[id-1]}.')
            return rigidbody_velocity

    # when the use this function, you prepare the 10 state + 1state of the rigid body
    # The function drops the last state and add the new state
    def rigidBodyVelocity(self, rigid_posestamped, state_index,id):
        """the function is to calculate the velocity of the rigid body

        Args:
            rigid_posestamped (object): all information of the rigid body from mocap system
            state_index (int): the index of the state buffer

        Returns:
            None: the function publish the velocity of the rigid body
        """
        spl=Spline_Calculator()
    
        # the init method had state buffer as np.arrange(10)
        # the buffer has own value and time as dictionary

        # the if is for the complate set of the state buffer (10 state)
        # the such transformation is for deque.rotate(-1)
        if state_index==self.state_buffer_index_len:
            dequw_state_buffer=deque(self.state_buffer[id-1])
            dequw_state_buffer.rotate(-1)
            transformed_dequw_state_buffer=list(dequw_state_buffer)[:-1]
            transformed_dequw_state_buffer.append([rigid_posestamped.pose, rigid_posestamped.header.stamp])
            self.state_buffer[id-1]=transformed_dequw_state_buffer
            #ipdb.set_trace()
            #TODO check the outputs
            # the out put is list
            rigidbody_velocity = spl.Smoothing_Spline(self.state_buffer[id-1])
            
            return rigidbody_velocity
            
        # for the preparation of the state buffer
        else:
            self.state_buffer[id-1][state_index]=rigid_posestamped.pose, rigid_posestamped.header.stamp
    ...
    def __unpackRigidBody(self, data):
        ...
        if (
            ...
            and self.__natNetStreamVersion[0] != 0
        ):
    ...
    def __unpackMocapData(self, data):
        ...
        for i in range(0, markerSetCount):
            # Model name
            modelName, separator, remainder = bytes(data[offset:]).partition(
                b"\0"
            )
        ...
        unlabeledMarkersCount = struct.unpack("i", data[offset : offset + 4])[
            0
        ]
        offset += 4
        ...
        if (
            self.__natNetStreamVersion[0] == 2
            and self.__natNetStreamVersion[1] > 0
        ) or self.__natNetStreamVersion[0] > 2:
        ...
        if (
            self.__natNetStreamVersion[0] == 2
            and self.__natNetStreamVersion[1] > 3
        ) or self.__natNetStreamVersion[0] > 2:
            labeledMarkerCount = struct.unpack("i", data[offset : offset + 4])[
                0
            ]
            offset += 4
        ...
        if (
            self.__natNetStreamVersion[0] == 2
            and self.__natNetStreamVersion[1] >= 9
        ) or self.__natNetStreamVersion[0] > 2:
            ...
            for i in range(0, forcePlateCount):
                ...
                # Channel Count
                forcePlateChannelCount = struct.unpack(
                ...
                for j in range(0, forcePlateChannelCount):
                    ...
                    forcePlateChannelFrameCount = struct.unpack(
                        "i", data[offset : offset + 4]
                    )[0]
                    offset += 4
                    for k in range(0, forcePlateChannelFrameCount):
                        forcePlateChannelVal = struct.unpack(
                            "i", data[offset : offset + 4]
                        )[0]
                        offset += 4
        ...
        if (
            self.__natNetStreamVersion[0] == 2
            and self.__natNetStreamVersion[1] >= 11
        ) or self.__natNetStreamVersion[0] > 2:
            ...
            for i in range(0, deviceCount):
                ...
                # Channel Count
                deviceChannelCount = struct.unpack(
                    "i", data[offset : offset + 4]
                )[0]
                offset += 4
                ...
                for j in range(0, deviceChannelCount):
                    ...
                    deviceChannelFrameCount = struct.unpack(
                        "i", data[offset : offset + 4]
                    )[0]
                    offset += 4
                    for k in range(0, deviceChannelFrameCount):
                        deviceChannelVal = struct.unpack(
                            "i", data[offset : offset + 4]
                        )[0]
                        offset += 4
        ...
        if (
            self.__natNetStreamVersion[0] == 2
            and self.__natNetStreamVersion[1] >= 7
        ) or self.__natNetStreamVersion[0] > 2:
        ...
        if (self.__natNetStreamVersion[0] >= 3) or major == 0:
            stampCameraExposure = struct.unpack(
                "ii", data[offset : offset + 8]
            )[0]
            offset += 8
            stampDataReceived = struct.unpack("ii", data[offset : offset + 8])[
                0
            ]
            offset += 8

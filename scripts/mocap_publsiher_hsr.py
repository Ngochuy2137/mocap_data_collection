...
class NatNetClient:
    def __init__( self ):
        ...
        self.pub_list = [
            rospy.Publisher('mocap_pose_topic/{}_pose'.format(rigid_body_list[i]), PoseStamped, queue_size=1)
            for i in range(len(rigid_body_list))
        ]

        #### Bernard ###
        self.bernard = rospy.Publisher('try',String, queue_size=1)
        ################

        #### Bernard ###
       #rospy.init_node('Bernard', anonymous=True)
       #rrate = rospy.Rate(10)
        ################

       #self.pub_list = [
        #    rospy.Publisher('mocap_pose_topic/test_pose', PoseStamped, queue_size=1)
        #]
    ...
    def rigidBodyListener( self, id, position, rotation, trackingValidFlag ):
        ...
        #print(rigid_posestamped)

        ## Bernard ####
        hello_str = 'hi'
        self.bernard.publish(hello_str)
       #rrate.sleep()
        ###############

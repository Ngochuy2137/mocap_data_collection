...
class NatNetClient:
    def __init__( self ):
        ...
        #rigid_body_list = ["go1_1", "go1_2", "go1_3"]
        rigid_body_list = ["go1_1","go1_2", "go1_3", "resource_1", "resource_2", "resource_3",  "resource_4", "resource_5", "resource_6", "tb3"]
    ...
    def rigidBodyListener( self, id, position, rotation, trackingValidFlag ):
        ...
        # Timestamp from mocap_data
        mocap_timestamp = current_ros_time

        # rospy.loginfo(f"Rigid body ID: {id}, ROS time: {current_ros_time}, Mocap time: {mocap_timestamp}")
        
        # Haal de naam van de rigid body op via de mapping
        rigid_body_name = self.rigid_body_to_name.get(id, f"unknown_{id}")
    
        #rospy.loginfo(f"Received rigid body ID: {id} and mapped to {rigid_body_name} with Position: {position}, Rotation: {rotation}")
        if self.pub_list[id-1].name[17:] == self.pub_list[-1].name[17:]:
            # making tf
            ...
            br.sendTransform(
                ...
                self.pub_list[id-1].name[17:]+"/odom",
